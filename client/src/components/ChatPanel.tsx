...
export default function ChatPanel({ roomId, open, onClose, mobile = false, className = '' }: ChatPanelProps) {
  ...
  const panelClasses = mobile
    ? 'h-full w-full'
    : 'h-full w-[320px] border-l border-[#334155]';

  return (
    <div
      className={[
        'flex flex-col bg-[#1E293B] text-white',
        panelClasses,
              ...
              <React.Fragment key={message.id}>
                {showDateSeparator && currentLabel && (
                  <div className="my-3 flex items-center justify-center">
                    <span className="rounded-full bg-[#334155] px-3 py-1 text-xs font-semibold text-slate-200">{currentLabel}</span>
                  </div>
                )}
